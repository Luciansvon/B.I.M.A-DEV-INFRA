...
1. Complete the bounded `prek` increment from Issue #3 by merging PR #5 after its final hosted run.
2. Continue Issue #3 P0 sequentially with action-validator, then pinact, zizmor, canonical evidence, and the agent packet. Add one bounded tool increment at a time with measured value.
