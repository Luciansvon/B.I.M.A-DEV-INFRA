...
1. Complete the bounded `prek` increment from Issue #3 with local Windows and hosted evidence.
2. Continue Issue #3 P0 sequentially: action-validator, pinact, zizmor, then canonical evidence and the agent packet. Add one bounded tool increment at a time with measured value.
