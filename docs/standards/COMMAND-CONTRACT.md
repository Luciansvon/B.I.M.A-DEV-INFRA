...
| `task check` | dependencies above | combined pre-flight, test, audit, canonical evidence, and lint results | any component fails |
...
GitHub Actions installs Task v3.53.1 with the upstream archive checksum, then calls the same `task test`, `task audit`, `task evidence`, and `task workflow-lint` entry points. The workflow retains its Ubuntu/Windows audit matrix and adds canonical files to the existing evidence artifacts. The adapter handles setup and upload; Task owns the deterministic command contract.
